...
- [ ] **标签置换检验** — 随机打乱 forward returns，因子如果仍然"有效"，说明是过拟合
- [ ] **时间序列打乱** — 破坏时序结构（block shuffle），检验因子是否依赖真实时序模式
- [ ] **随机股票池** — 从全市场随机抽样构造伪 universe，因子在伪池上不应显著
- [ ] **噪声注入** — 对因子值加高斯噪声，评估 IC 衰减速率（衰减快=信号脆弱）

实现方式：新建 `adversarial_validator.py`，作为 `anti_overfit.py` 的补充层。Cloud 验证作为最终确认。
